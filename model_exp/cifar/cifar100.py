...
class MNIST:
    def __init__(self):
        ...
        elif self.cfg.arch == 'VGG':
            self.model = VGG()
        elif self.cfg.arch == 'CCVGG':
            self.model = CCVGG()
        elif self.cfg.arch == 'VGGModel':
            self.model = VGGModel()
        elif self.cfg.arch == 'CCVGGSigmoid':
            self.model = CCVGGSigmoid()
        elif self.cfg.arch == 'VGGModelSigmoid':
            self.model = VGGModelSigmoid()
        elif self.cfg.arch == 'CCVGGLeaky':
            self.model = CCVGG_Leaky()
        elif self.cfg.arch == 'VGGModelLeaky':
            self.model = VGGModel_Leaky()
        else:
            self.model = Linear(width=self.cfg.width, depth=self.cfg.depth)
    ...
    def add_arguments(self):
        ...
        model_names = ['MLP', 'LinearModel', 'Linear','CCMLP', 'CCLinearModel', 'VGG', 'CCVGG', 'VGGModel', 'CCVGGSigmoid', 'VGGModelSigmoid', 'CCVGGLeaky', 'VGGModelLeaky']
        ...
        parser.set_defaults(dataset='cifar100', workers=1, batch_size=[64, 1000])
